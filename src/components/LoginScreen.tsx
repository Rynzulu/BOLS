...
export function LoginScreen({
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (user.email === "tycodona@gmail.com") {
        onLogin({
          role: "super_admin",
          adminName: user.displayName ? `${user.displayName} (Super Admin)` : "Super Administrator (tycodona@gmail.com)"
        });
      } else {
        onLogin({
          role: "school_admin",
          adminName: user.displayName || user.email || "School Administrator"
        });
      }
    } catch (err: any) {
