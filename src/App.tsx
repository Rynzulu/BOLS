...
export default function App() {
  ...
  const [session, setSession] = useState<UserSession | null>(
    initialRoute.autoLogin && initialResolved.session ? initialResolved.session : null
  );
  const [activeTab, setActiveTab] = useState<string>(
    initialResolved.targetTab || "Dashboard"
  );
    ...
    const sub = subscribeToRealtimeSchoolData({
      ...
      onStudentsChange: (cloudStudents) => {
        if (cloudStudents) {
          ...
          try {
            localStorage.setItem("zambian_school_students", JSON.stringify(cloudStudents));
          } catch (_) {}
        }
      },
      onTeachersChange: (cloudTeachers) => {
        if (cloudTeachers) {
          ...
          try {
            localStorage.setItem("zambian_school_teachers", JSON.stringify(cloudTeachers));
          } catch (_) {}
        }
      },
      ...
      onClassesChange: (cloudClasses) => {
        if (cloudClasses) {
          ...
          try {
            localStorage.setItem("zambian_school_classes", JSON.stringify(cloudClasses));
          } catch (_) {}
        }
      },
      ...
      onSubjectsChange: (cloudSubjects) => {
        if (cloudSubjects) {
          setSubjectsCatalog(cloudSubjects);
        }
      },
      ...
      onFeesChange: (cloudFees) => {
        if (cloudFees) {
          setFees(cloudFees);
          try {
            localStorage.setItem("zambian_school_fees", JSON.stringify(cloudFees));
          } catch (_) {}
        }
      },
      ...
      onEventsChange: (cloudEvents) => {
        if (cloudEvents) {
          ...
          try {
            localStorage.setItem("zambian_school_events", JSON.stringify(cloudEvents));
          } catch (_) {}
        }
      },
      onMessagesChange: (cloudMessages) => {
        if (cloudMessages) {
          ...
          try {
            localStorage.setItem("zambian_school_messages", JSON.stringify(cloudMessages));
          } catch (_) {}
        }
      },
  ...
  const handleDirectDemoLogin = (role: RoleType, user?: string, studentId?: number) => {
    if (role === "admin" || role === "super_admin" || role === "school_admin") {
      ...
      const s = studentId
        ...
        : students[0];
      setSession({ role: "student", student: s });
      setActiveTab("ReportCards");
    }
  ...
  // Teacher restricted view for assigned classes, batches, and pupils
  ...
  const visibleClasses = useMemo(() => {
    if (isRoleTeacher && session?.teacher) {
      ...
      return classes.filter(
        c => teacherAssignedClassIds.includes(c.id) ||
             c.teacherId === session.teacher?.id ||
             (c.teacherName && c.teacherName.toLowerCase() === teacherName)
      );
    }
    return classes;
  }, [classes, isRoleTeacher, session?.teacher, teacherAssignedClassIds]);
  ...
  const visibleBatches = useMemo(() => {
    if (isRoleTeacher && session?.teacher) {
      ...
      return batches.filter(
        b => b.leadTeacherId === session.teacher?.id ||
    ...
    return batches;
  }, [batches, visibleClasses, isRoleTeacher, session?.teacher]);

  const visibleStudents = useMemo(() => {
    if (isRoleTeacher && session?.teacher) {
      const visibleClassIds = visibleClasses.map(c => c.id);
      return students.filter(s => visibleClassIds.includes(s.classId));
    }
    if (fixedStudentId) {
      return students.filter(s => s.id === fixedStudentId);
    }
    return students;
  }, [students, visibleClasses, isRoleTeacher, session?.teacher, fixedStudentId]);
  ...
  const handleAddRoute = (route: Omit<TransportRoute, "id">) => {
    setTransportRoutes(prev => [...prev, { ...route, id: Date.now() }]);
    showToast("Bus route created.");
  };
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
          ...
          {activeTab === "Transport" && (
            <TransportManagementModule
              ...
              students={students}
              onAddVehicle={(v) => {
                setTransportVehicles(prev => [v, ...prev]);
                showToast(`Vehicle ${v.registrationNumber} registered.`);
              }}
              onAssignPupil={(p) => {
                setTransportPupils(prev => [p, ...prev]);
                showToast(`Assigned ${p.studentName} to bus route.`);
              }}
              onRemovePupil={(id) => {
                setTransportPupils(prev => prev.filter(a => a.id !== id));
